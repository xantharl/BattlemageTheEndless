...
void UProjectileManagerComponent::Initialize(ACharacter* character)
{
...
void UProjectileManagerComponent::SpawnProjectiles_Actor(
...
TArray<FTransform> UProjectileManagerComponent::GetSpawnLocations(const FProjectileConfiguration& configuration, const FTransform& rootTransform, UAttackBaseGameplayAbility* spawningAbility)
{
...
void UProjectileManagerComponent::GetRingPoints(const FProjectileConfiguration& configuration, const FTransform& spawnTransform, FTransformArrayA2& returnArray, bool bInwards)
{
...
void UProjectileManagerComponent::GetLinePoints(const FProjectileConfiguration& configuration, FTransformArrayA2& returnArray, FTransform& spawnTransform)
{
	auto const bEvenAmount = configuration.Amount % 2 == 0;
...
void UProjectileManagerComponent::OnProjectileDestroyed(AActor* destroyedActor)
{
	//destroyedActor->Destroy();
}

FTransform UProjectileManagerComponent::GetOwnerLookAtTransform()
{
	if (!OwnerCharacter)
	{
		UE_LOG(LogTemp, Error, TEXT("OwnerCharacter not set for projectile manager, cannot get lookat transform"));
